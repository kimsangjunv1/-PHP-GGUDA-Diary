...
<?php
    ...
    switch($searchOption){
        case "title":
            $sql .= "WHERE t.tipTitle LIKE '%{$searchKeyword}%' ORDER BY myTipID DESC ";
            break;
        case "content":
            $sql .= "WHERE t.tipContents LIKE '%{$searchKeyword}%' ORDER BY myTipID DESC ";
            break;
        case "name":
            $sql .= "WHERE m.youName LIKE '%{$searchKeyword}%' ORDER BY myTipID DESC ";
            break;
    }
    ...
    if($result){
        ...
        if($count > 0){
            for($i=1; $i <= $count; $i++){
                ...
                echo "<p class='contents_boardId'>".$info['myTipID']."</p>";
...
<?php
    ...
    if($page != 1){
        $prevPage = $page - 1;
        echo "<li><a href='tipSearch.php?page=1&searchKeyword={$searchKeyword}&searchOption={$searchOption}'>처음</a></li>";
        echo "<li><a href='tipSearch.php?page={$prevPage}'>이전</a></li>";
    } 
    ...
    for($i=$startPage; $i<=$endPage; $i++){
        ...
        echo "<li class='{$active}'><a href='tipSearch.php?page={$i}&searchKeyword={$searchKeyword}&searchOption={$searchOption}'>{$i}</a></li>";
    }

    //다음 페이지, 마지막 페이지
    if($page != $endPage) {
        $nextPage = $page + 1;
        echo "<li><a href='tipSearch.php?page={$nextPage}&searchKeyword={$searchKeyword}&searchOption={$searchOption}'>다음</a></li>";
        echo "<li><a href='tipSearch.php?page={$eventCount}&searchKeyword={$searchKeyword}&searchOption={$searchOption}'>마지막</a></li>";
    }
?>
                    </ul>
                </div>
