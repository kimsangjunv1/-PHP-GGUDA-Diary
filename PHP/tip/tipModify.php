            ...
            <div class="board">
                ...
                <div class="section_selector">
                    ...
                    <div class="modify_cont">
                        <a class="select remove_btn" href="tipDelete.php?myTipID=<?=$_GET['myTipID']?>" onclick="alert('정말 삭제하시겠습니까? ;3')">삭제(다른방식)</a>
                    </div>
                </div>
                <hr>
                <div class="board__view">
                <form action="tipModifySave.php" name="tipModify" method="post">
                        <fieldset>
                            <legend class="blind">게시판 작성 영역</legend>
<?php
    $myTipID = $_GET['myTipID'];
    $sql = "SELECT myTipID, tipTitle, tipContents FROM myTip WHERE myTipID = {$myTipID}";
    $result = $connect -> query($sql);

    if($result){
        $info = $result -> fetch_array(MYSQLI_ASSOC);
        echo "<div style='display:none'><label for='myBoardID'>번호</label><input type='text' name='myTipID' id='myBoardID' value='".$info['myTipID']."'></div><div><label for='boardTitle' class='blind'>제목</label><input type='text' name='tipTitle' id='boardTitle' value='".$info['tipTitle']."'></div>";
...
<?php
    ...
    // echo $myBoardID;
    $sql = "SELECT t.tipTitle, t.tipSection, m.youImageFile, t.regTime, t.tipView, t.tipContents FROM myTip t JOIN myMember m ON(m.myMemberID = t.myMemberID) WHERE t.myTipID = {$myTipID}";
    $result = $connect -> query($sql);
...
?>
<?php
    echo "<div><label for='boardContents' class='blind'>내용</label><textarea name='tipContents' id='boardContents' rows='20'>".$info['tipContents']."</textarea></div>";
?>
                            <div>
<?php
    if($result){
...
?>
                        ...
                        <div class="wail">
                            ...
                            <!-- <a class="select remove_btn" href="boardDelete.php?myBoardID=<?=$_GET['myTipID']?>" onclick="alert('정말 삭제하시겠습니까? ;3')">삭제(다른방식)</a> -->
                        </div>
                </div>
            </div>
        </div>
